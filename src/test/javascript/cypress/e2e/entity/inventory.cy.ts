...
describe('Inventory e2e test', () => {
  const inventoryPageUrl = '/hostelmgmt/inventory';
  const inventoryPageUrlPattern = new RegExp('/hostelmgmt/inventory(\\?.*)?$');
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/hostelmgmt/api/inventories+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/hostelmgmt/api/inventories').as('postEntityRequest');
    cy.intercept('DELETE', '/services/hostelmgmt/api/inventories/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (inventory) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/hostelmgmt/api/inventories/${inventory.id}`,
      }).then(() => {
        inventory = undefined;
      });
    }
  ...
  it('Inventories menu should load Inventories page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('hostelmgmt/inventory');
  ...
  describe('Inventory page', () => {
    describe('create button click', () => {
      ...
      it('should load create Inventory page', () => {
        ...
        cy.url().should('match', new RegExp('/hostelmgmt/inventory/new$'));
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/hostelmgmt/api/inventories',
  ...
  describe('new Inventory page', () => {
    ...
    it('should create an instance of Inventory', () => {
      cy.get(`[data-cy="name"]`).type('android rant');
      cy.get(`[data-cy="name"]`).should('have.value', 'android rant');

      cy.get(`[data-cy="quantity"]`).type('1050');
      cy.get(`[data-cy="quantity"]`).should('have.value', '1050');
