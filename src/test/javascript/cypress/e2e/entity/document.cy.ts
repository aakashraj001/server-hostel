...
describe('Document e2e test', () => {
  const documentPageUrl = '/hostelmgmt/document';
  const documentPageUrlPattern = new RegExp('/hostelmgmt/document(\\?.*)?$');
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/hostelmgmt/api/documents+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/hostelmgmt/api/documents').as('postEntityRequest');
    cy.intercept('DELETE', '/services/hostelmgmt/api/documents/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (document) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/hostelmgmt/api/documents/${document.id}`,
      }).then(() => {
        document = undefined;
      });
    }
  ...
  it('Documents menu should load Documents page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('hostelmgmt/document');
  ...
  describe('Document page', () => {
    describe('create button click', () => {
      ...
      it('should load create Document page', () => {
        ...
        cy.url().should('match', new RegExp('/hostelmgmt/document/new$'));
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/hostelmgmt/api/documents',
          body: documentSample,
        }).then(({ body }) => {
          document = body;
  ...
  describe('new Document page', () => {
    ...
    it('should create an instance of Document', () => {
      cy.get(`[data-cy="type"]`).type('past');
      cy.get(`[data-cy="type"]`).should('have.value', 'past');
