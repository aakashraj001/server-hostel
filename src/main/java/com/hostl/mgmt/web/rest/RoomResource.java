...
/**
 * REST controller for managing {@link com.hostl.mgmt.domain.Room}.
 */
@RestController
...
public class RoomResource {
    ...
    private static final String ENTITY_NAME = "hostelMgmtRoom";
     ...
     */
    @GetMapping("")
    public List<Room> getAllRooms(@RequestParam(name = "filter", required = false) String filter) {
        if ("champ-is-null".equals(filter)) {
            log.debug("REST request to get all Rooms where champ is null");
            return StreamSupport.stream(roomRepository.findAll().spliterator(), false).filter(room -> room.getChamp() == null).toList();
        }
