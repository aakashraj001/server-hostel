...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class PaymentService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/payments', 'hostelmgmt');
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      date: restPayment.date ? dayjs(restPayment.date) : undefined,
    };
  }
