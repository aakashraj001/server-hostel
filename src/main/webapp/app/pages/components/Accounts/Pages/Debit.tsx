...
const Debit = () => {
  ...
  const topcolor = '#42FFB0';
  const bottomcolor = ' #43F58A';
  const borderColors = '#4EFFB5';
  const pointerBoderColors = '#4EFFB5';
  ...
  const { debitExpand, debitPage, setdebitPage, setdebitExpand } = useContext(ContextData);
  ...
  return (
    <section>
      <div className={`${debitExpand ? 'hidden' : 'grid'} grid-rows-5 gap-3  mt-[9.5vh] h-fit `}>
      ...
      <div>{debitPage && <Debitdetails props={debitpaymnetdetails} />}</div>
      <div className=" w-full h-screen">{debitExpand && <ExpandPage heading="Debit History" props={credit} />}</div>
    </section>
  );
};
